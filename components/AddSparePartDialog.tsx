...
import {
  createSparePart,
  getVoitureModelsForConteneur,
} from "@/lib/actions/subcase";
import {
  Package,
  Save,
  Loader2,
...
import { translateText } from "@/lib/actions/translation";
...
const AddSparePartDialog: React.FC<AddSparePartDialogProps> = ({
  ...
  const [voitureModels, setVoitureModels] = useState<
  ...
  useEffect(() => {
    if (
      ...
      const translatePartName = async () => {
        ...
        try {
          const result = await translateText(formData.partName.trim());
          if (
            result.success &&
            result.data &&
            result.data !== formData.partName
          ) {
            setFormData((prev) => ({
              ...prev,
              partNameFrench: result.data || "",
            }));
    ...
    <Dialog open={open} onOpenChange={handleClose}>
      <DialogContent className="max-w-3xl max-h-[96vh] overflow-hidden flex flex-col p-0 gap-0 shadow-2xl">
          ...
          <div className="relative flex items-center gap-5">
            ...
            <div className="flex-1">
              ...
              <DialogDescription className="text-purple-100 text-base">
                Remplissez le formulaire pour enregistrer une nouvelle pièce de
                rechange
              </DialogDescription>
            </div>
          </div>
                ...
                <Switch
                  ...
                  onCheckedChange={(checked) => {
                    ...
                    if (checked && formData.partName.trim()) {
                      setIsTranslating(true);
                      isTranslationUpdate.current = true;
                      // Assuming translateText is defined elsewhere and returns a Promise
                      // For example:
                      // const translateText = async (text: string) => { /* ... */ };
                      // If translateText is not defined, this will cause a runtime error.
                      // I'm adding it here based on the provided snippet.
                      ...
                      translateText(formData.partName.trim())
                        .then((result) => {
                          if (
                            ...
                            toast.success(
                              "Traduction automatique activée et effectuée",
                              {
                                icon: (
                      ...
                      toast.info(
                        "Mode manuel activé - Vous pouvez modifier le champ français",
                        {
        ...
        <form
          ...
          className="flex-1 overflow-y-auto custom-scrollbar px-8 py-6 space-y-6 bg-gradient-to-b from-gray-50 via-white to-gray-50"
        >
          ...
          <div className="space-y-5">
            ...
            <div className="space-y-2.5">
              <Label
                htmlFor="partCode"
                className="text-sm font-semibold text-gray-800 flex items-center gap-2.5"
              >
              ...
              <div className="space-y-2.5">
                ...
                <Select
                  ...
                  onValueChange={(value) => {
                    ...
                    if (errors.voitureModelId)
                      setErrors({ ...errors, voitureModelId: "" });
                  }}
                  disabled={loadingModels}
                >
                  <SelectTrigger
                    className={`h-12 text-base transition-all duration-200 shadow-sm w-full ${
