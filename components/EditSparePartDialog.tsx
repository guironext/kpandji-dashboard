    ...
    if (open) {
      if (sparePart) {
        ...
        setFormData({
          ...
          verificationName:
            sparePart.verificationName ||
            `${sparePart.partCode}${sparePart.partName}`,
        });
        ...
        if (!hasFrenchTranslation && sparePart.partName.trim()) {
          setTimeout(async () => {
            ...
            isTranslationUpdate.current = true;
            try {
              const result = await translateText(sparePart.partName.trim());
              if (
                result.success &&
                result.data &&
                result.data !== sparePart.partName
              ) {
                setFormData((prev) => ({
                  ...prev,
  ...
  useEffect(() => {
    if (
      ...
      const translateOnEnable = async () => {
        ...
        if (
          !formData.partNameFrench.trim() ||
          formData.partNameFrench === formData.partName
        ) {
          setIsTranslating(true);
          isTranslationUpdate.current = true;
          try {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (result.success) {
        ...
        toast.error(
          result.error ||
            `Erreur lors de l'${isEditMode ? "édition" : "enregistrement"}`,
          {
      ...
      console.error(
        `Error ${isEditMode ? "updating" : "creating"} spare part:`,
        error,
      );
      toast.error(
        `Erreur lors de l'${isEditMode ? "édition" : "enregistrement"}`,
                ...
                {autoTranslateEnabled && !isManualFrenchEdit && (
                  <Button
                    ...
                    onClick={() => {
                      ...
                      toast.info(
                        "Mode édition manuelle activé - Vous pouvez maintenant modifier le champ français",
                        {
            ...
            <div className="space-y-2.5">
              ...
              <Input
                ...
                className={`h-12 text-base transition-all duration-200 shadow-sm bg-gray-50 border-2 border-gray-300 text-gray-700 cursor-not-allowed ${
                  errors.verificationName ? "border-red-400 bg-red-50/50" : ""
                }`}
              />
