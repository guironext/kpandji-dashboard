export type StatutTacheActiviteProjetRoutine =
  | "NOUVEAU"
  | "EN_ATTENTE"
  | "EN_COURS"
...
export type TacheStatutColumn = {
  value: StatutTacheActiviteProjetRoutine;
  label: string;
  shortLabel: string;
  progress: number;
  headerClass: string;
...
export const TACHE_STATUT_COLUMNS: TacheStatutColumn[] = [
  {
    value: "NOUVEAU",
    label: "Nouveau",
    shortLabel: "Nouveau",
...
const STATUT_MAP = Object.fromEntries(
  TACHE_STATUT_COLUMNS.map((c) => [c.value, c])
) as Record<StatutTacheActiviteProjetRoutine, TacheStatutColumn>;

export function getTacheStatutConfig(
  statut: StatutTacheActiviteProjetRoutine
): TacheStatutColumn {
  return STATUT_MAP[statut] ?? STATUT_MAP.NOUVEAU;
}

export function getTacheStatutProgress(statut: StatutTacheActiviteProjetRoutine): number {
  return getTacheStatutConfig(statut).progress;
}

export function getTacheTimeProgress(
  dateDebut: string,
...
export const ALL_TACHE_STATUT_VALUES = TACHE_STATUT_COLUMNS.map((c) => c.value);

export function isStatutTacheActiviteProjetRoutine(
  value: string
): value is StatutTacheActiviteProjetRoutine {
  return ALL_TACHE_STATUT_VALUES.includes(value as StatutTacheActiviteProjetRoutine);
}
