...
import { orderDepotageForConteneur } from "@/lib/actions/conteneur";
...
const ConteneurArrivesClient: React.FC<Props> = ({ conteneurs }) => {
  const router = useRouter();
  const [loadingConteneurId, setLoadingConteneurId] = useState<string | null>(null);

  const handleOrderDepotage = async (conteneurId: string) => {
    setLoadingConteneurId(conteneurId);
    try {
      const result = await orderDepotageForConteneur(conteneurId);
      if (result.success) {
        toast.success(result.message || "Dépotage ordonné avec succès");
        router.refresh();
      } else {
        toast.error(result.error || "Erreur lors de l'ordonnancement du dépotage");
      }
    } catch (error) {
      console.error("Error ordering depotage:", error);
      toast.error("Une erreur est survenue lors de l'ordonnancement du dépotage");
    } finally {
  ...
  const renderConteneur = (conteneur: ConteneurType, index: number) => {
    return (
      <div
        key={conteneur.id}
        className={`group relative overflow-hidden bg-gradient-to-br from-white via-green-50/30 to-emerald-50/20 border-2 border-green-200/60 hover:border-green-400/80 p-6 md:p-8 rounded-3xl transition-all duration-500 hover:shadow-2xl hover:-translate-y-2 backdrop-blur-sm`}
        style={{
      ...
      >
        ...
        <div
          className={`absolute top-0 right-0 w-64 h-64 bg-gradient-to-br from-green-200/20 to-emerald-200/20 rounded-full blur-3xl opacity-0 group-hover:opacity-100 transition-opacity duration-500`}
        ></div>
          ...
          <div className="flex flex-col lg:flex-row lg:items-start lg:justify-between gap-6">
            <div className="flex items-center gap-5 justify-between flex-1">
              <div className="relative">
                <div
                  className={`absolute inset-0 bg-gradient-to-br from-green-400 to-emerald-400 rounded-2xl blur-lg opacity-50 group-hover:opacity-75 transition-opacity`}
                ></div>
                <div
                  className={`relative bg-gradient-to-br from-green-100 via-emerald-100 to-teal-100 p-5 rounded-2xl shadow-lg group-hover:scale-110 transition-transform duration-300 border-2 border-green-200/50`}
                >
              ...
              <div className="flex-1 space-y-3">
                <div className="flex items-center gap-3 flex-wrap">
                  ...
                  <Badge
                    className={`bg-gradient-to-r from-green-500 to-emerald-600 text-white text-xs px-4 py-1.5 shadow-lg font-semibold`}
                  >
                    {conteneur.etapeConteneur}
                  </Badge>
                  <Button
                    onClick={() => handleOrderDepotage(conteneur.id)}
                    disabled={loadingConteneurId === conteneur.id}
                    className="group/btn relative overflow-hidden bg-gradient-to-r from-blue-500 via-indigo-500 to-purple-500 hover:from-blue-600 hover:via-indigo-600 hover:to-purple-600 text-white shadow-lg hover:shadow-xl px-4 py-2 rounded-xl font-semibold transition-all duration-300 hover:scale-105 border-0 disabled:opacity-50 disabled:cursor-not-allowed"
                  >
                    <div className="absolute inset-0 bg-gradient-to-r from-white/20 to-transparent opacity-0 group-hover/btn:opacity-100 transition-opacity duration-300"></div>
                    <div className="relative flex items-center gap-2">
                      <Truck className="h-4 w-4 group-hover/btn:scale-110 transition-transform duration-300" />
                      <span className="text-sm md:text-base">
                        {loadingConteneurId === conteneur.id ? "Traitement..." : "Ordonner Dépotage"}
                      </span>
                    </div>
                  </Button>
                </div>
                <div className="flex flex-wrap items-center gap-3 md:gap-4">
                  <div
                    className={`flex items-center gap-2 bg-gradient-to-r from-green-50 to-emerald-100 px-4 py-2 rounded-xl border border-green-200/50 shadow-sm`}
                  >
                ...
                {/* Additional Info */}
                <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 pt-4 border-t border-green-100/50">
        ...
        <div className="space-y-5 relative z-10">
          <div className="flex items-center justify-between mb-5">
            <h4 className="font-bold text-gray-900 text-xl md:text-2xl flex items-center gap-3">
              ...
              <Badge
                variant="secondary"
                className="ml-2 bg-gradient-to-r from-green-100 to-emerald-100 text-green-700 border-green-200 text-sm px-4 py-1 font-semibold"
              >
          ...
          {conteneur.commandes.length > 0 ? (
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4 md:gap-6">
              {conteneur.commandes.map((commande, cmdIndex) => (
                <div
                  key={commande.id}
                  className="group/commande relative overflow-hidden bg-gradient-to-br from-white via-blue-50/40 to-purple-50/30 rounded-2xl p-5 md:p-6 border-2 border-green-100/60 hover:border-green-300 hover:shadow-xl transition-all duration-300 transform hover:-translate-y-1"
                  style={{
                    animationDelay: `${cmdIndex * 50}ms`,
                  }}
                >
                  <div className="absolute top-0 right-0 w-32 h-32 bg-gradient-to-br from-green-200/20 to-emerald-200/20 rounded-full blur-2xl opacity-0 group-hover/commande:opacity-100 transition-opacity duration-300"></div>

                  <div className="flex justify-between items-start mb-4 relative z-10">
                    <div className="flex items-start gap-3 flex-1 min-w-0">
                      <div className="bg-gradient-to-br from-green-100 via-emerald-100 to-teal-100 p-3 rounded-xl group-hover/commande:scale-110 transition-transform duration-300 border border-green-200/50">
                    ...
                    <div className="flex flex-col gap-2 ml-3">
                      <Badge className="bg-gradient-to-r from-green-500 to-emerald-500 text-white text-xs px-3 py-1 shadow-md font-semibold">
                  ...
                  <div className="bg-gradient-to-br from-green-50/60 via-emerald-50/40 to-teal-50/30 rounded-xl p-4 border border-green-100/60 relative z-10">
                    <div className="grid grid-cols-2 gap-3 text-xs md:text-sm">
                      ...
                      <div className="flex items-center gap-2 col-span-2 pt-2 border-t border-green-100/50">
                      ...
                      {commande.spareParts.length > 0 && (
                        <div className="col-span-2 pt-2 border-t border-green-100/50">
                          <div className="flex items-center gap-2 mb-2">
                            <Package className="h-4 w-4 text-green-600" />
                            <span className="text-gray-600 font-semibold">
                              Pièces Arrivées ({commande.spareParts.length}):
                            </span>
                          </div>
            ...
            <div className="text-center py-16 bg-gradient-to-br from-gray-50 via-green-50/30 to-emerald-50/20 rounded-2xl border-2 border-dashed border-gray-300">
              ...
              <p className="text-gray-600 font-semibold text-lg">
                Aucune commande arrivée dans ce conteneur
              </p>
            </div>
          )}
        </div>
          ...
          <div className="mt-6 space-y-5 relative z-10">
            <div className="flex items-center justify-between mb-5">
              <h4 className="font-bold text-gray-900 text-xl md:text-2xl flex items-center gap-3">
                ...
                Sous-caisses avec Pièces Arrivées
              </h4>
            </div>
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-slate-50 via-green-50/40 to-emerald-50/30 relative overflow-hidden">
      ...
      <div className="relative z-10 max-w-[1920px] mx-auto p-4 md:p-6 lg:p-8 space-y-6 md:space-y-8">
        ...
        <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-6">
          <div className="flex items-center gap-4">
            <div className="relative">
              <div className="absolute inset-0 bg-gradient-to-br from-green-600 via-emerald-600 to-teal-600 rounded-3xl blur-xl opacity-60 animate-pulse"></div>
              <div className="relative bg-gradient-to-br from-green-600 via-emerald-600 to-teal-600 p-4 md:p-5 rounded-3xl shadow-2xl transform hover:scale-105 transition-all duration-300">
                <CheckCircle2 className="h-7 w-7 md:h-8 md:w-8 text-white" />
              </div>
            </div>
            <div className="space-y-1">
              <h1 className="text-3xl md:text-4xl lg:text-5xl font-extrabold bg-gradient-to-r from-gray-900 via-green-800 to-emerald-800 bg-clip-text text-transparent tracking-tight">
                Conteneurs Arrivés
              </h1>
              <div className="flex items-center gap-2 text-gray-600">
                <div className="h-2 w-2 bg-green-500 rounded-full animate-pulse"></div>
                <p className="text-sm md:text-base font-medium">
                  Conteneurs, commandes et pièces avec statut ARRIVE
                </p>
              </div>
            </div>
          </div>
          {stats.totalConteneurs > 0 && (
            <div className="flex items-center gap-3 px-4 py-2 bg-white/80 backdrop-blur-md rounded-2xl shadow-lg border border-green-200/50">
              ...
              <span className="text-sm font-semibold text-gray-700">
                ...
                {stats.totalConteneurs !== 1 ? "s" : ""} arrivé
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 md:gap-6">
          ...
          <Card className="group relative overflow-hidden border-0 shadow-xl hover:shadow-2xl transition-all duration-500 bg-gradient-to-br from-green-500 via-emerald-600 to-teal-600 text-white transform hover:-translate-y-2">
            ...
            <CardContent className="p-6 relative z-10">
              ...
              <p className="text-green-200 text-xs">Arrivés</p>
            </CardContent>
          </Card>
          ...
          <Card className="group relative overflow-hidden border-0 shadow-xl hover:shadow-2xl transition-all duration-500 bg-gradient-to-br from-purple-500 via-purple-600 to-fuchsia-600 text-white transform hover:-translate-y-2">
            ...
            <CardContent className="p-6 relative z-10">
              ...
              <p className="text-purple-200 text-xs">Arrivées</p>
            </CardContent>
          </Card>
          ...
          {/* Total SpareParts */}
          <Card className="group relative overflow-hidden border-0 shadow-xl hover:shadow-2xl transition-all duration-500 bg-gradient-to-br from-indigo-500 via-indigo-600 to-purple-600 text-white transform hover:-translate-y-2">
            <div className="absolute inset-0 bg-gradient-to-br from-white/10 to-transparent opacity-0 group-hover:opacity-100 transition-opacity duration-500"></div>
            <CardContent className="p-6 relative z-10">
              <div className="flex items-center justify-between mb-4">
                <div className="bg-white/20 backdrop-blur-sm p-3 rounded-2xl group-hover:scale-110 transition-transform duration-300">
              ...
              <p className="text-indigo-200 text-xs">Arrivées</p>
            </CardContent>
          </Card>
        </div>
        ...
        <Card className="shadow-2xl border-0 overflow-hidden bg-white/95 backdrop-blur-xl transform transition-all duration-300 hover:shadow-3xl">
          <CardHeader className="relative bg-gradient-to-r from-green-600 via-emerald-600 to-teal-600 text-white border-0 pb-8 pt-8 overflow-hidden">
              ...
              {conteneurs.length > 0 ? (
                ...
                <div className="flex flex-col items-center justify-center py-24 md:py-32">
                  <div className="relative mb-8">
                    <div className="absolute inset-0 bg-gradient-to-br from-green-200/30 to-emerald-200/30 rounded-full blur-2xl opacity-50 animate-pulse"></div>
                    <div className="relative bg-gradient-to-br from-green-50 to-emerald-50 p-10 rounded-full shadow-inner border-2 border-green-200/50">
                      <CheckCircle2 className="h-24 w-24 text-green-400" />
                    </div>
                  </div>
                  <p className="text-center font-extrabold text-2xl md:text-3xl mb-3 bg-gradient-to-r from-green-600 to-emerald-600 bg-clip-text text-transparent">
                    Aucun conteneur trouvé
                  </p>
                  <p className="text-center text-base md:text-lg text-gray-500 max-w-md">
                    Aucun conteneur avec statut ARRIVE pour le moment.
                  </p>
                  <div className="mt-6 px-6 py-3 bg-green-50 border border-green-200 rounded-xl">
                    <p className="text-sm text-green-700 font-medium">
                      Les conteneurs apparaîtront ici une fois qu&apos;ils
                      seront marqués comme &quot;ARRIVE&quot; avec leurs commandes et pièces.
                    </p>
                  </div>
                </div>
              )}
...
export default ConteneurArrivesClient;
