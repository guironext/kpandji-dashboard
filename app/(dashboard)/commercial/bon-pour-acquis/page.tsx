...
export default function Page() {
  ...
  const handleBonPourAcquis = async () => {
    const currentFacture = currentData[0];
    if (!currentFacture) return;

    const result = await updateBonPourAcquis(currentFacture.id, true);
    if (result.success) {
      toast.success("Bon pour Acquis activé avec succès");
      // Update local state
      setFactures((prev) =>
        prev.map((f) =>
          f.id === currentFacture.id
            ? { ...f, bon_pour_acquis: true }
            : f
        )
      );
    } else {
        ...
        <div className="bg-white rounded-lg shadow-2xl p-8">
          <div className="flex w-full justify-between mb-6 print-hide">
            <div className="flex gap-4">
              ...
              <Button
                onClick={handleBonPourAcquis}
                disabled={currentData.length === 0 || currentData[0]?.bon_pour_acquis === true}
