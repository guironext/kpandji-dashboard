...
import { generateNextNumeroBonPourAccord, getBonPourAccordByFactureId } from "@/lib/actions/bonpouraccord";
...
export default function Page() {
  ...
  useEffect(() => {
    const fetchNumero = async () => {
      ...
      const result = await getBonPourAccordByFactureId(currentFacture.id);
      if (result.success && result.data) {
        setNumero((result.data as { numero: string }).numero);
      } else {
        setNumero("");
      }
    };
  ...
  const handleGenerateNumero = async () => {
    ...
    const result = await generateNextNumeroBonPourAccord(currentFacture.id);
    if (result.success && result.data) {
  ...
  const handleDelete = async () => {
    ...
    if (
      confirm(
        `Êtes-vous sûr de vouloir supprimer ce bon pour accord (${currentFacture.id.slice(-7)}) ?`
      )
    ) {
          ...
          <div id="printable-area">
            ...
            {factures.length === 0 && (
              <div className="text-center py-12">
                <p className="text-lg text-gray-600">Aucun bon pour accord trouvé (client entreprise uniquement)</p>
              </div>
            )}
          </div>
