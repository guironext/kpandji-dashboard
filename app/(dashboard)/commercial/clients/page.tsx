...
const ClientPage = () => {
	...
	const filteredAndSortedClients = useMemo(() => {
		const filtered = clients.filter((client) => {
			...
			const matchesSector =
				filterSector === "all" || client.secteur_activite === filterSector;
	...
	return (
		<div className="min-h-screen bg-gradient-to-br from-purple-50 via-pink-50 to-blue-50 p-3 sm:p-4 md:p-6 space-y-4 sm:space-y-6">
			...
			<div className="relative z-10 space-y-6">
				...
				<div className="relative">
					...
					<div className="relative bg-white/80 backdrop-blur-xl rounded-2xl sm:rounded-3xl p-4 sm:p-6 md:p-8 shadow-2xl border border-purple-200/50">
						<div className="flex flex-col lg:flex-row lg:items-center justify-between gap-4 sm:gap-6">
							<div className="space-y-3 min-w-0 flex-1">
								<div className="flex flex-col sm:flex-row sm:items-center gap-3 sm:gap-4">
									<div className="relative shrink-0">
										...
										<div className="relative p-3 sm:p-4 bg-gradient-to-br from-purple-500 to-pink-500 rounded-2xl shadow-lg">
											<Users className="w-8 h-8 sm:w-10 sm:h-10 text-white" />
										</div>
									</div>
									<div className="min-w-0">
										<div className="flex flex-wrap items-center gap-2 sm:gap-3">
											<h1 className="text-2xl sm:text-3xl md:text-4xl lg:text-5xl font-black bg-gradient-to-r from-purple-600 via-pink-600 to-blue-600 bg-clip-text text-transparent leading-tight">
												Gestion des Clients
											</h1>
											<Sparkles className="w-6 h-6 sm:w-8 sm:h-8 text-pink-500 animate-pulse shrink-0" />
										</div>
										<p className="text-purple-700/70 text-sm sm:text-base md:text-lg font-medium flex flex-wrap items-center gap-2 mt-2">
											<UserCheck className="w-4 h-4 sm:w-5 sm:h-5 shrink-0" />
											<span>Gérez votre portefeuille client efficacement</span>
										</p>
									</div>
								</div>
							</div>
							...
							<Dialog open={showForm} onOpenChange={setShowForm}>
								...
								<DialogContent className="max-h-[min(90dvh,90vh)] overflow-y-auto bg-gradient-to-br from-purple-50 to-pink-50 border-2 border-purple-200 p-4 sm:p-6 sm:max-w-4xl">
									<DialogHeader>
									...
									<form onSubmit={handleSubmit} className="space-y-6">
										...
										<div className="flex flex-col-reverse gap-2 sm:flex-row sm:justify-end sm:gap-3 pt-4 border-t-2 border-purple-200">
											<Button
												type="button"
												variant="outline"
												onClick={resetForm}
												className="h-12 w-full sm:w-auto px-6 rounded-xl border-2 border-purple-300 hover:bg-purple-50">
												Annuler
											</Button>
											<div className="relative w-full sm:w-auto">
												...
												<Button
													type="submit"
													className="relative h-12 w-full sm:w-auto px-8 font-bold bg-gradient-to-r from-purple-500 to-pink-500 hover:from-purple-600 hover:to-pink-600 text-white border-0 rounded-xl">
						...
						<CardContent className="p-4 sm:p-6">
							<div className="flex flex-col lg:flex-row gap-3 sm:gap-4 min-w-0">
								<div className="relative flex-1 min-w-0">
									<Search className="absolute left-3 sm:left-4 top-1/2 transform -translate-y-1/2 text-purple-500 w-4 h-4 sm:w-5 sm:h-5 pointer-events-none" />
									<Input
										placeholder="🔍 Rechercher par nom, entreprise ou téléphone..."
										value={searchTerm}
										onChange={(e) => setSearchTerm(e.target.value)}
										className="pl-10 sm:pl-12 h-12 sm:h-14 border-2 border-purple-200 focus:border-purple-500 focus:ring-purple-500 rounded-2xl text-sm sm:text-base font-medium min-w-0"
									/>
								</div>

								<div className="flex flex-col sm:flex-row gap-3 w-full lg:w-auto lg:shrink-0 min-w-0">
									<Select value={filterSector} onValueChange={setFilterSector}>
										<SelectTrigger className="w-full sm:min-w-[12rem] sm:w-56 h-12 sm:h-14 border-2 border-purple-200 rounded-2xl font-semibold min-w-0">
									...
									<Select
										...
										<SelectTrigger className="w-full sm:min-w-[12rem] sm:w-56 h-12 sm:h-14 border-2 border-purple-200 rounded-2xl font-semibold min-w-0">
											<SortAsc className="w-5 h-5 mr-2 text-pink-500 shrink-0" />
				...
				<div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-4 sm:gap-6">
					{filteredAndSortedClients.map((client) => (
						<div key={client.id} className="group relative min-w-0">
							...
							<Card className="relative border-2 border-purple-200/50 shadow-2xl bg-gradient-to-br from-white to-purple-50/30 rounded-2xl sm:rounded-3xl overflow-hidden transition-all duration-300 sm:hover:scale-[1.02]">
								<div className="absolute top-0 left-0 right-0 h-2 bg-gradient-to-r from-purple-500 via-pink-500 to-blue-500"></div>
								<CardContent className="p-4 sm:p-6">
									<div className="flex flex-col gap-3 sm:flex-row sm:justify-between sm:items-start mb-4 min-w-0">
										<div className="flex-1 min-w-0">
											<h3 className="font-bold text-lg sm:text-xl bg-gradient-to-r from-purple-700 to-pink-700 bg-clip-text text-transparent mb-2 break-words">
												{client.nom}
											</h3>
											{client.entreprise && (
										...
										<div className="flex flex-wrap items-center gap-2 shrink-0 sm:justify-end">
											<Badge className="bg-gradient-to-r from-purple-100 to-pink-100 text-purple-800 border-purple-300 border-2 px-2 sm:px-3 py-1 font-bold shadow-sm">
											...
											<DropdownMenu>
												<DropdownMenuTrigger asChild>
													<Button
														variant="ghost"
														size="sm"
														className="h-9 w-9 p-0 rounded-xl hover:bg-purple-100">
							...
							<CardContent className="p-8 sm:p-12 md:p-16">
								<div className="text-center">
									<div className="relative w-20 h-20 sm:w-24 sm:h-24 mx-auto mb-4 sm:mb-6">
										...
										<div className="relative w-20 h-20 sm:w-24 sm:h-24 bg-gradient-to-br from-purple-100 to-pink-100 rounded-full flex items-center justify-center">
											<Users className="w-10 h-10 sm:w-12 sm:h-12 text-purple-600" />
										</div>
									</div>
									<h3 className="text-xl sm:text-2xl font-bold mb-3 bg-gradient-to-r from-purple-600 to-pink-600 bg-clip-text text-transparent px-1">
										{searchTerm || filterSector !== "all"
											? "🔍 Aucun résultat"
											: "👋 Aucun client trouvé"}
									</h3>
									<p className="text-sm sm:text-base mb-6 sm:mb-8 max-w-md mx-auto text-purple-700/70 font-medium px-2">
										{searchTerm || filterSector !== "all"
									...
									{!searchTerm && filterSector === "all" && (
										<div className="relative inline-block">
											...
											<Button
												...
												className="relative h-12 sm:h-14 w-full max-w-sm px-6 sm:px-8 text-base sm:text-lg font-bold bg-gradient-to-r from-purple-500 to-pink-500 hover:from-purple-600 hover:to-pink-600 text-white border-0 rounded-2xl shadow-xl">
