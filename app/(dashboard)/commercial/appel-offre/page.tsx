...
export default function Page() {
  ...
  const [evolutionSteps, setEvolutionSteps] = useState<EvolutionAppelOffreStep[]>([]);
  ...
  useEffect(() => {
    const fetchNumero = async () => {
      ...
      if (result.success && result.data) {
        const data = result.data as { numero: string };
  ...
  const handleSaveEvolution = async () => {
    ...
    if (!numero) {
      toast.error("Générez d'abord le numéro de l'appel d'offre avant d'enregistrer");
      return;
    }
  ...
  const handleExportToWord = async () => {
    ...
    try {
      ...
      const docChildren: (Paragraph | DocxTable)[] = [
        ...
        new Paragraph({ text: "APPEL D'OFFRE", alignment: AlignmentType.CENTER, spacing: { after: 100 } }),
      ];
      ...
      docChildren.push(
        ...
        new Paragraph({ text: "3️⃣ Détails de l'appel d'offre", spacing: { before: 100, after: 100 } }),
        new DocxTable({ rows: tableRows, width: { size: 100, type: "pct" } }),
        new Paragraph({ text: "4️⃣ Conditions de l'offre", spacing: { before: 300, after: 100 } }),
        new Paragraph({ text: `Date de l'appel d'offre : ${formatDateDDMMYYYY(facture.date_facture)}` }),
        ...
        new Paragraph({ text: "Validité de l'offre : 30 jours à compter de la date d'émission", spacing: { after: 200 } }),
        new Paragraph({ text: "Remarque : Cette offre constitue notre proposition commerciale en réponse à votre appel d'offres. Elle engage KPANDJI AUTOMOBILES dès sa signature.", spacing: { after: 300 } }),
          ...
          <div id="printable-area" className="min-w-0">
            ...
            {currentData.map((facture: Facture) => (
              <div key={facture.id}>
                ...
                <div className="mb-2 min-w-0">
                  <h2 className="text-xs font-bold text-gray-800 mb-2 sm:text-sm">3️⃣ Détails de l&apos;appel d&apos;offre</h2>
                ...
                <div className="bg-gray-50 border-2 border-gray-300 rounded-lg p-3 sm:p-4 mb-2">
                  <h2 className="text-xs font-bold text-gray-800 mb-3 sm:text-sm">4️⃣ Conditions de l&apos;offre</h2>
                  <div className="grid grid-cols-1 gap-3 text-[11px] sm:grid-cols-2 sm:gap-2 sm:text-xs">
                    <div className="flex flex-col gap-0.5 sm:flex-row sm:flex-wrap sm:gap-x-2">
                      <p className="shrink-0 font-semibold">Date de l&apos;appel d&apos;offre :</p>
                    ...
                    <div className="flex flex-col gap-0.5 sm:flex-row sm:flex-wrap sm:gap-x-2 sm:col-span-2">
                      <p className="shrink-0 font-semibold">Validité de l&apos;offre :</p>
                      <p className="min-w-0 break-words">30 jours à compter de la date d&apos;émission</p>
                    </div>
                  </div>
                  <div className="mt-3 p-2 bg-green-50 border border-green-300 rounded">
                    <p className="text-xs">
                      <span className="font-bold">Remarque :</span> Cette offre constitue notre proposition commerciale en réponse à votre appel d&apos;offres. Elle engage KPANDJI AUTOMOBILES dès sa signature.
                    </p>
                  </div>
                </div>
            ...
            {factures.length === 0 && (
              <div className="text-center py-12">
                <p className="text-lg text-gray-600">Aucun appel d&apos;offre trouvé (client entreprise uniquement)</p>
              </div>
            )}
          </div>
