...
export default function Page() {
	...
	const getEffectiveTotalTtc = (facture: Facture) =>
		editedTotalTtc[facture.id] ?? facture.total_ttc;
	...
	useEffect(() => {
		const fetchData = async () => {
			...
			if (facturesResult.success && facturesResult.data) {
				console.log(
					"Factures with accessoires:",
					JSON.stringify(
						facturesResult.data.map((f) => ({
							id: f.id,
			...
			if (accessoiresResult.success && accessoiresResult.data) {
				console.log(
					"All accessoires:",
					JSON.stringify(accessoiresResult.data, null, 2),
				);
				setAccessoires(
					accessoiresResult.data.map((acc) => ({
						id: acc.id,
						nom: acc.nom,
						prix: acc.prix ?? null,
	...
	const handlePrint = () => {
      ...
      `;
		...
		const conditionsTextColor = conditionsTextHidden ? "#ffffff" : "#000000";
		const conditionsTitleColor = conditionsTextHidden ? "#ffffff" : "#ea580c";
            ...
            @media print {
              * {
              ...
              .footer-conditions-title {
                padding: 0 !important;
                margin: 0 !important;
                color: ${conditionsTitleColor} !important;
              }
              .footer-conditions p {
                padding: 0 !important;
                margin: 0 !important;
                color: ${conditionsTextColor} !important;
              }
              .footer-info {
                background-color: #f0fdf4 !important;
          ...
          <div class="page">
            <div class="content-wrapper">
                ...
                Arrêter la présente facture à la somme de <span>${(() => {
									const amountText =
										...
										numberToFrench(
											Math.floor(
												convertAmount(
													(editedTotalTtc?.[currentFacture.id] ??
														currentFacture.total_ttc) ||
														0,
												),
											),
										);
              ...
              <div class="notes-section">
                ...
                ${(() => {
									const notes =
										notesProforma?.[currentFacture.id] ??
										currentFacture.notes_proforma ??
										"";
									return notes
										? `<p style="font-weight: 500; margin-bottom: 8px; white-space: pre-wrap;">${escapeHtml(notes)}</p>`
										: "";
								})()}
            ...
            <div class="footer">
              <div class="footer-conditions">
                ...
                <p style="color: ${conditionsTextColor};">60% d'accompte à la commande</p>
                <p style="font-weight: 300; color: ${conditionsTextColor};">DELAIS DE PRODUCTION ET DE LIVRAISON: 4 MOIS</p>
                <p style="color: ${conditionsTextColor};">SOLDE avant livraison</p>
              </div>
              <p style="color: #000; font-size: 8px; margin: 0;">
                <span style="font-weight: bold;">Garantie: </span>
				...
				if (updatedFactures.success && updatedFactures.data) {
					...
					if (
						...
						setCurrentPage(
							Math.max(
								1,
								Math.ceil(updatedFactures.data.length / itemsPerPage),
							),
						);
					}
				}
	...
	const handleApplyCurrency = () => {
		...
		if (curr.code !== "XOF" && (!rate || isNaN(rate) || rate <= 0)) {
			toast.error(
				"Veuillez entrer un taux de change valide (1 " +
					curr.symbol +
					" = X FCFA)",
			);
			return;
		...
		toast.success(
			curr.code === "XOF"
				? "Affichage en Francs CFA"
				: `Devise changée en ${curr.label}`,
		);
	};
			...
			<Dialog open={showCurrencyDialog} onOpenChange={setShowCurrencyDialog}>
				<DialogContent className="sm:max-w-md">
					<DialogHeader>
						...
						<DialogDescription>
							Choisissez une devise pour afficher les montants. Les montants
							sont convertis depuis les Francs CFA (FCFA).
						</DialogDescription>
					</DialogHeader>
					<div className="flex flex-col gap-4 py-4">
						<div className="flex flex-col gap-2">
							<Label>Devise</Label>
							<Select
								value={currencyDialogSelection}
								onValueChange={setCurrencyDialogSelection}>
								<SelectTrigger>
								...
								<SelectContent>
									{CURRENCIES.map((c) => (
										<SelectItem key={c.code} value={c.code}>
											{c.label}{" "}
											{c.code !== "XOF" && `(1 ${c.symbol} ≈ ${c.rate} FCFA)`}
										</SelectItem>
									))}
								</SelectContent>
							</Select>
							...
							<div className="flex flex-col gap-2">
								...
								<div className="flex items-center gap-2">
									<span className="text-sm text-muted-foreground">
										1{" "}
										{
											CURRENCIES.find((c) => c.code === currencyDialogSelection)
												?.symbol
										}{" "}
										=
									</span>
					...
					<div id="printable-area">
								...
								<div className="mb-4">
									<Table className="rounded-lg overflow-hidden">
										<TableHeader>
											<TableRow className="bg-green-50 border-b border-black">
												...
												<TableHead className="text-black font-bold text-right no-wrap">
													Prix Unitaire HT ({getCurrencyLabel()})
												</TableHead>
												<TableHead className="text-right text-black font-bold no-wrap">
													Total HT ({getCurrencyLabel()})
												</TableHead>
											</TableRow>
										</TableHeader>
										<TableBody>
														...
														<TableRow
															...
															<TableCell className="text-black flex flex-col gap-y-1 text-lg font-semibold">
																...
																{accessoire.description && (
																	<p className="text-[9px] font-normal text-black max-w-80 text-wrap">
																		{accessoire.description}
																	</p>
																)}
															</TableCell>
													...
													<TableRow className="bg-white border-b border-orange-200">
														...
														<TableCell className="text-black flex flex-col gap-y-1 text-lg font-semibold">
															...
															{facture.accessoire_description && (
																<p className="text-[7px] font-normal text-black max-w-80 text-wrap">
																	{facture.accessoire_description}
																</p>
															)}
														</TableCell>
														...
														<TableCell className="text-black text-right text-sm pr-6">
															{formatAmount(facture.accessoire_prix || 0)}
														</TableCell>
													</TableRow>
												)}
										</TableBody>
										<TableFooter className="text-sm border-t border-b border-black mt-4">
											...
											{facture.remise !== 0 && (
												<TableRow className="bg-white">
													...
													<TableCell className="text-right font-medium pr-6 text-black">
														{formatAmount(facture.montant_remise)}
													</TableCell>
												</TableRow>
											)}
											{facture.remise !== 0 && (
												<TableRow className="bg-green-50">
													...
													<TableCell className="text-right font-medium pr-6 text-black">
														{formatAmount(facture.montant_net_ht)}
													</TableCell>
												</TableRow>
											)}
											<TableRow className="bg-white">
													...
													{editingTotalTtc === facture.id ? (
														<div className="flex items-center justify-end gap-1 print-hide">
															<Input
																type="text"
																value={
																	totalTtcInputs[facture.id] ??
																	formatAmount(getEffectiveTotalTtc(facture))
																}
																onChange={(e) =>
																	setTotalTtcInputs({
																		...totalTtcInputs,
														...
														<div className="flex items-center justify-end gap-1">
															...
															<Button
																...
																onClick={() => {
																	...
																	setTotalTtcInputs({
																		...totalTtcInputs,
																		[facture.id]: formatAmount(
																			getEffectiveTotalTtc(facture),
																		),
																	});
																}}
													...
													<Input
														value={
															...
															numberToFrench(
																Math.floor(
																	convertAmount(
																		getEffectiveTotalTtc(facture) || 0,
																	),
																),
															)
														}
									...
									<div className="flex w-full justify-between mt-5 px-8">
										<div className="flex flex-col gap-2 flex-1 max-w-md">
											...
											<Textarea
												value={
													notesProforma[facture.id] ??
													facture.notes_proforma ??
													""
												}
												onChange={(e) =>
													setNotesProforma((prev) => ({
														...prev,
														[facture.id]: e.target.value,
													}))
												}
											...
											<Button
												...
												onClick={async () => {
													...
													const notes =
														...
														facture.notes_proforma ??
														"";
													const result = await updateProformaNotes(
														facture.id,
														notes,
													);
													setSavingNotes(null);
													if (result.success) {
														...
														const updatedFactures = await getFacturesByUser(
															clerkId!,
														);
														if (
															updatedFactures.success &&
															updatedFactures.data
														) {
															setFactures(
																updatedFactures.data as unknown as Facture[],
															);
														}
													} else {
														toast.error(
						...
						<div className="print-footer flex flex-col w-full bottom-0 right-0 left-0">
							<div
								role="button"
								tabIndex={0}
								onClick={() => setConditionsTextHidden((prev) => !prev)}
								onKeyDown={(e) => {
									if (e.key === "Enter" || e.key === " ")
										setConditionsTextHidden((prev) => !prev);
								}}
								title={
									conditionsTextHidden
										? "Cliquer pour afficher les conditions"
										: "Cliquer pour masquer les conditions"
								}
								className={`flex flex-col w-full mb-2 rounded-b-lg text-[9px] cursor-pointer select-none transition-colors ${
									conditionsTextHidden
										? "ring-1 ring-dashed ring-orange-300 bg-orange-50/40 print:ring-0 print:bg-transparent"
										: "hover:bg-orange-50/30"
								}`}>
								<p
									className={`font-bold mt-2 ${conditionsTextHidden ? "text-white" : "text-orange-600"}`}>
									CONDITIONS:
								</p>

								<p className={conditionsTextHidden ? "text-white" : "text-black"}>
									60% d&apos;accompte à la commande
								</p>
								<p
									className={`font-semibold ${conditionsTextHidden ? "text-white" : "text-black"}`}>
							...
							<p className="text-black">
								<span className="font-bold">Garantie: </span>3 Ans ou 100.000
								kilometres
							</p>
