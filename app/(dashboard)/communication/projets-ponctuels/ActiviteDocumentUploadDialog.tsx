...
import {
  getActiviteDocuments,
  uploadActiviteDocument,
  type ProjetPonctuelDocumentItem,
} from "@/lib/actions/projet-ponctuel-document";
...
const ACCEPTED_DOCUMENT_TYPES =
  ".pdf,.doc,.docx,.jpg,.jpeg,.png,.webp,.xls,.xlsx,.ppt,.pptx,.txt";

type Props = {
  open: boolean;
  onOpenChange: (open: boolean) => void;
  activite: ProjetPonctuelActiviteItem | null;
  onDocumentUploaded?: (document: ProjetPonctuelDocumentItem) => void;
};
...
export default function ActiviteDocumentUploadDialog({
  open,
  onOpenChange,
  activite,
  onDocumentUploaded,
}: Props) {
  const [documents, setDocuments] = useState<ProjetPonctuelDocumentItem[]>([]);
  ...
  const loadDocuments = useCallback(async () => {
    ...
    try {
      const result = await getActiviteDocuments(activite.id);
      if (result.success) {
  ...
  }, [open, activite?.id, loadDocuments]);
  ...
  const handleUpload = async () => {
    if (!activite || !file || isUploading) return;
    ...
    try {
      ...
      formData.append("activiteId", activite.id);
      formData.append("nom", trimmedNom);
      formData.append("typeDocument", typeDocument);
      formData.append("file", file);

      const result = await uploadActiviteDocument(formData);
      if (result.success) {
      ...
      >
        ...
        <div className="min-h-0 flex-1 space-y-4 overflow-y-auto px-5 py-4">
          <div className="space-y-2">
            <Label htmlFor="doc-nom" className="text-xs font-semibold text-slate-700">
              Nom du document
            </Label>
            <Input
              id="doc-nom"
          ...
          <div className="space-y-2">
            ...
            {isLoadingDocs ? (
              ...
              <p className="rounded-xl border border-dashed border-slate-200 bg-slate-50/60 px-3 py-4 text-center text-xs text-slate-500">
                Aucun document pour cette activité.
              </p>
            ) : (
              <div className="space-y-2">
                {documents.map((doc) => (
