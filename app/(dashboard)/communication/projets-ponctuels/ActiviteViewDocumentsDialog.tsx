...
import {
  getActiviteDocumentsForCreator,
...
export default function ActiviteViewDocumentsDialog({
  open,
  onOpenChange,
  activite,
}: Props) {
  ...
  const loadDocuments = useCallback(async () => {
    ...
    setIsLoading(true);
    try {
      const result = await getActiviteDocumentsForCreator(activite.id);
      if (result.success) {
          ...
          <div className="flex items-start gap-3">
            ...
            <div className="min-w-0 flex-1">
              ...
              <DialogDescription className="mt-0.5 text-sm text-slate-600">
                {activite?.titre ?? "Activité"} — fichiers chargés par le responsable
              </DialogDescription>
            </div>
          </div>
        </div>
          ...
          {isLoading ? (
            ...
            <div className="flex h-40 flex-col items-center justify-center gap-2 text-center text-slate-500">
              ...
              <p className="text-xs text-slate-400">
                Le responsable n&apos;a pas encore déposé de fichier.
              </p>
            </div>
          ) : (
