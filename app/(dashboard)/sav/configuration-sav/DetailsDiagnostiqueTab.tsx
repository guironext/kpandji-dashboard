...
export default function DetailsDiagnostiqueTab({ embedded = false }: { embedded?: boolean }) {
  ...
  return (
    <div className={embedded ? "space-y-3 sm:space-y-5" : "space-y-6 animate-in fade-in duration-500"}>
      <div className="flex flex-col gap-3 sm:gap-5 lg:flex-row lg:items-center lg:justify-between">
        ...
        {embedded && details.length > 0 && (
          <div className="flex items-center gap-2 rounded-xl border border-amber-100 bg-amber-50/80 px-3 py-2">
            ...
            <span className="truncate text-xs font-semibold tabular-nums text-amber-900 sm:text-sm">
              Catalogue : {formatPrixFCFA(details.reduce((sum, d) => sum + (Number(d.prix_unitaire) || 0), 0))}
            </span>
          </div>
        )}

        <div className="flex flex-col items-stretch gap-2.5 sm:flex-row sm:items-center sm:gap-3 lg:ml-auto">
          <div className="relative">
            ...
            <Input
              ...
              className="h-11 rounded-xl border-slate-200 bg-slate-50/50 pl-9 focus:bg-white focus:ring-2 focus:ring-violet-500/20 sm:h-10 sm:w-[260px]"
            />
          </div>
          {categories.length > 0 && (
            <div className="flex items-center gap-2">
              ...
              <Select value={categoryFilter} onValueChange={setCategoryFilter}>
                <SelectTrigger className="h-11 w-full rounded-xl border-slate-200 bg-slate-50/50 focus:bg-white sm:h-10 sm:w-[200px]">
          ...
          <Button
            ...
            size="default"
            className="h-11 w-full shrink-0 rounded-xl bg-gradient-to-r from-violet-600 to-purple-600 px-6 font-medium text-white shadow-lg shadow-violet-500/25 hover:from-violet-700 hover:to-purple-700 sm:h-10 sm:w-auto"
          >
      ...
      <div
        className={
          embedded
            ...
            : "overflow-hidden rounded-2xl border border-slate-200/80 bg-white shadow-lg shadow-slate-200/30 ring-1 ring-slate-100"
        }
      >
        {loading ? (
          <div className="flex flex-col items-center justify-center py-32 gap-6">
            ...
            <div className="text-center py-12 px-5 sm:py-20 sm:px-8">
