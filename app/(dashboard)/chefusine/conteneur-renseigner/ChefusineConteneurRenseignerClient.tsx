...
const ChefusineConteneurRenseignerClient: React.FC<Props> = ({ data }) => {
  ...
  const stats = useMemo(() => {
    ...
    const totalCommandes =
      totalCommandesInConteneurs + totalStandaloneCommandes;
    ...
    const totalDisponiblesStandalone = commandes.filter(
      (cmd) => cmd.commandeFlag === "DISPONIBLE",
    ).length;
    const totalDisponibles =
      totalDisponiblesInConteneurs + totalDisponiblesStandalone;
              ...
              <div className="flex-1 space-y-3">
                ...
                <div className="flex items-center justify-between ">
                  ...
                  <Button
                    onClick={() =>
                      router.push(
                        `/chefusine/conteneur-renseigner/${conteneur.id}`,
                      )
                    }
  ...
  const hasData =
    stats.totalConteneurs > 0 || stats.totalStandaloneCommandes > 0;
          ...
          {hasData && (
            <div className="flex items-center gap-3 px-4 py-2 bg-white/80 backdrop-blur-md rounded-2xl shadow-lg border border-purple-200/50">
              ...
              <span className="text-sm font-semibold text-gray-700">
                ...
                {stats.totalConteneurs !== 1 ? "s" : ""} et{" "}
                {stats.totalStandaloneCommandes} commande
                {stats.totalStandaloneCommandes !== 1 ? "s" : ""} à renseigner
              </span>
            </div>
          )}
        </div>
